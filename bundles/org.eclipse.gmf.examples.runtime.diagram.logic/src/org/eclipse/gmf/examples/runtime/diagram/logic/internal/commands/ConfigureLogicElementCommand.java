...
/**
 * Abstract superclass for commands that configure <code>Element</code> s in the
 * logic model with input and output terminals.
 *
 * @author ldamus
 */
public abstract class ConfigureLogicElementCommand extends ConfigureElementCommand {
	 ...
	 * @param configurableType the kind of element that can be configured by this
	 *                         command instance.
	 */
	 ...
	 *
	 * @param logicElement    the logic element
	 * @param id              the terminal identifier
	 * @param progressMonitor the monitor to measure progress through long-running
	 *                        operations
	 * @return the new terminal
	 */
	protected InputTerminal createInputTerminal(Element logicElement, String id, IProgressMonitor progressMonitor) {

		return (InputTerminal) createTerminal(LogicSemanticType.INPUT_TERMINAL, logicElement, id, progressMonitor);
	}
	...
	protected OutputTerminal createOutputTerminal(Element logicElement, String id, IProgressMonitor progressMonitor) {
		...
		return (OutputTerminal) createTerminal(LogicSemanticType.OUTPUT_TERMINAL, logicElement, id, progressMonitor);
	}
	 ...
	 * @return the new terminal
	 */
	protected InputOutputTerminal createInputOutputTerminal(Element logicElement, String id,
			IProgressMonitor progressMonitor) {

		return (InputOutputTerminal) createTerminal(LogicSemanticType.INPUT_OUTPUT_TERMINAL, logicElement, id,
				progressMonitor);
	}
	 ...
	 *
	 * @param elementType     the type of terminal to create
	 * @param logicElement    the logic element
	 * @param id              the terminal identifier
	 * @param progressMonitor the monitor to measure progress through long-running
	 *                        operations
	 * @return the new terminal
	 */
	private Terminal createTerminal(IElementType elementType, Element logicElement, String id,
			IProgressMonitor progressMonitor) {

		Terminal terminal = createTerminal(elementType, logicElement, progressMonitor);
	 ...
	 * @param progressMonitor the monitor to measure progress through long-running
	 *                        operations
	 * @return the new terminal element, or <code>null</code> if it wasn't created
	 */
	private Terminal createTerminal(IElementType terminalType, Element logicElement, IProgressMonitor progressMonitor) {

		Element result = createElement(logicElement, terminalType, TERMINALS, progressMonitor);
	...
	/**
	 * Creates a <code>Wire</code> from <code>source</code> to <code>target</code>.
	 *
	 * @param source          the source terminal
	 * @param target          the target terminal
	 * @param progressMonitor the monitor to measure progress through long-running
	 *                        operations
	 * @return the new <code>Wire</code>, or <code>null</code> if none was created
	 */
	protected Wire createWire(OutputTerminal source, InputTerminal target, IProgressMonitor progressMonitor) {

		CreateRelationshipRequest createRequest = new CreateRelationshipRequest(getEditingDomain(), source, target,
		...
		if (elementType != null) {
			...
			if (createCommand != null && createCommand.canExecute()) {
				...
				try {
					...
					Log.error(LogicDiagramPlugin.getInstance(), LogicDiagramStatusCodes.COMMAND_FAILURE,
	 ...
	 *
	 * @param container       the conainer element
	 * @param gateType        the kind of gate to create
	 * @param progressMonitor progressMonitor the monitor to measure progress
	 *                        through long-running operations
	 * @return the new <code>Gate</code>, or <code>null</code> if none was created
	 */
	protected Gate createGate(ContainerElement container, IElementType gateType, IProgressMonitor progressMonitor) {

		Element result = createElement(container, gateType, null, progressMonitor);
	 ...
	 *
	 * @param container          the container for the new element
	 * @param type               the kind of new element to create
	 * @param containmentFeature the feature in which to put the new element
	 * @param progressMonitor    progressMonitor the monitor to measure progress
	 *                           through long-running operations
	 * @return the new <code>Element</code>, or <code>null</code> if none was
	 *         created
	 */
	private Element createElement(Element container, IElementType type, EReference containmentFeature,
			IProgressMonitor progressMonitor) {

		CreateElementRequest createRequest = new CreateElementRequest(getEditingDomain(), container, type,
				containmentFeature);
	 ...
	 *
	 * @param elementType     the type of terminal
	 * @param terminal        the terminal element
	 * @param id              the terminal identifier
	 * @param progressMonitor the monitor to measure progress through long-running
	 *                        operations
	 */
	private void setTerminalId(IElementType elementType, Terminal terminal, String id,
			IProgressMonitor progressMonitor) {

		SetRequest setRequest = new SetRequest(getEditingDomain(), terminal, SemanticPackage.eINSTANCE.getTerminal_Id(),
				id);
