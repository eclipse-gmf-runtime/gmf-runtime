...
public class LogicNotationEditor extends FileDiagramEditorWithFlyoutPalette {
	...
	protected void initializeGraphicalViewer() {
		...
		getDiagramGraphicalViewer().addDropTargetListener((TransferDropTargetListener) new DiagramDropTargetListener(
				getDiagramGraphicalViewer(), LocalTransfer.getInstance()) {
			@Override
			protected List getObjectsBeingDropped() {
				TransferData[] data = getCurrentEvent().dataTypes;
				List eObjects = new ArrayList();

				for (TransferData element : data) {
					if (LocalTransfer.getInstance().isSupportedType(element)) {
						IStructuredSelection selection = (IStructuredSelection) LocalTransfer.getInstance()
								.nativeToJava(element);
					...
					if (modelObj instanceof EObject) {
						...
						for (Object o : eObjects) {
							if (!(o instanceof EObject)
									|| !SUPPORTED_DRAG_DROP_ECLASSES.contains(((EObject) o).eClass())) {
								return false;
							}
						}
	...
	protected EditPartFactory getOutlineViewEditPartFactory() {
		return new EditPartFactory() {
			...
			public EditPart createEditPart(EditPart context, Object model) {
				...
				} else if (model instanceof View && ViewType.GROUP.equals(((View) model).getType())) {
