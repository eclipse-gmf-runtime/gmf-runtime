...
public class PasteDelegate extends AbstractClipboardDelegate {
	...
	protected void doRun(final Clipboard clipboard) {
		...
		command.append(new RecordingCommand(domain, "", //$NON-NLS-1$
				new Runnable() {
					@Override
					public void run() {
	...
	void doRun(Clipboard clipboard, Collection objectsPasted) {
		...
		EmfTransferType data = (EmfTransferType) clipboard.getContents(EmfTransfer.getInstance());
		...
		if (data != null) {
			...
			for (Iterator iter = getSelectedObjects().iterator(); iter.hasNext();) {
				...
				Collection pasted = ClipboardUtil.pasteElementsFromString(clipString, target, null, null);
	...
	/**
	 * A command that selects a collection of elements in the editor whenever it is
	 * executed or redone.
	 */
