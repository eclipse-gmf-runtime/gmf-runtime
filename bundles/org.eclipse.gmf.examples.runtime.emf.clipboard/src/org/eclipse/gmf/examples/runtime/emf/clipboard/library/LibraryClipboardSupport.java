 ...
 * already in the destination library, then we don't create a duplicate but
 * merge the new author with the existing one. This effectively adds all of the
 * new author's books that we have copied from another library to the existing
 * author who, having the same name, is assumed to be the same person</li>
...
class LibraryClipboardSupport extends AbstractClipboardSupport {
	...
	/**
	 * Provide a mapping of name attributes for the <code>EClass</code>es of the
	 * Library metamodel.
	 */
	@Override
