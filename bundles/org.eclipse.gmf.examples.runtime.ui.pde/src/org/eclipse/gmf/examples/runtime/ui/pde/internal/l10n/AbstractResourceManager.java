...
/**
 * <p>
 * AbstractResourceManager is an abstract class which is designed to encapsulate
 * a behaviour of an object that provides a single point of access to the I18N
 * and resource management per logical cluster of Java packages.
 * </p>
 * <p>
 * The logical cluster, though can be equivalent, but not limited to one per
 * plug-in. For instance, a logical cluster in the plug-in devoted to UML
 * diagrams can be split into as many logical clusters, as there are types of
 * diagrams in the plug-in. That is - a plugin that is dedicated to
 * collaboration and sequence diagrams, can be split into two logical clusters,
 * each requiring separate resource management - one for collaboration diagram,
 * and another for sequence diagram.
 * </p>
 * <p>
 * Each cluster should allocate a dedicated package for I18N and resource
 * management. The preferred convention is use i18n package suffix to designate
 * such package. For example, com.ibm.diagrams.collaboration.l10n and
 * com.ibm.diagrams.sequence.l10n are designated packages for resource and I18N
 * management.
 * </p>
 * <p>
 * Each cluster-oriented resource management package will have a single
 * <code>org.eclipse.gmf.examples.ui.pde.internal.l10n.AbstractResourceManager</code>
 * subclass. The subclass should have a singleton instance and override, if
 * necessary, the resource initialization method <code>
 * 	initializeMessageResources()
 * </code> to add the assignments of values to any value holding static
 * variables declared in the subclass. The subclass can be designated to hold
 * any resource and I18N related static variables if necessary, e.g. message
 * resource bundle keys.
 *
 * The <code>initilaizeResources()</code> method should be overridden by
 * subclasses in order to add/remove initialization of any additional/redundant
 * resource types
 * </p>
 * <p>
 * The subclass should override if necessary the default names of the message
 * resource bundle file (default MessageBundle) using the method: <code>
 * 		getMessageBundleName()
 * </code>
 * </p>
 * <p>
 * On the file system side the convention is to store all resources associated
 * with the given cluster in the designated i18n package. That includes
 * messages.properties. For example, com.ibm.diagram.collaboration cluster will
 * store its resources here: com/ rational/ diagram/ collaboration/ l10n/
 * messages.properties
 * </p>
 * <p>
...
public abstract class AbstractResourceManager {
	 ...
	 * subclasses must override if name of the messages bundle is different from the
	 * default
	 */
	...
	/**
	 * Returns the messageBundle. Resource bundles contain locale-specific objects -
	 * text, numbers, etc.
	 * 
	...
	/**
	 * Returns the name of the messages bundle, including the package path. E.g. for
	 * MessageBundle.properties file located at com.ibm.diagrams.collaboration.l10n
	...
	/**
	 * Load various resources. Do nothing by default. Subclasses should override
	 * this method to include initialization of the particular resource types.
	 */
	protected abstract void initializeResources();

	/**
	 * Populate messageBundle with text related resources from the MessageBundle
	 * properties file. This method provides single assignment point to the private
	 * variable messagesBundle. To override default initialization subclasses should
	 * override createMessagesBundle()
	 * 
	...
	protected ResourceBundle createMessagesBundle() {
		try {
			...
			Trace.catching(GmfExamplesPlugin.getDefault(), GmfExamplesDebugOptions.EXCEPTIONS_CATCHING, getClass(),
	...
	/**
	 * Returns the string from the plugin's resource bundle, or defaultValue if not
	 * found.
	 * 
	 * @return - value for the given key or the suuplied default if value was not
	 *         found
	 * @param key          java.lang.String the key to retrieve the value
	 * @param defaultValue java.lang.String the default value to return if no value
	 *                     by the given key was found
	 */
	public String getString(String key, String defaultValue) {
		try {
			...
			Log.warning(GmfExamplesPlugin.getDefault(), GmfExamplesStatusCodes.L10N_FAILURE,
					MessageFormat.format(MISSING_RESOURCE_MESSAGE, new Object[] { key }), mre);
	...
	public String formatMessage(String patternKey, Object[] args) {
		...
		try {
			...
			Trace.catching(getPlugin(), GmfExamplesDebugOptions.EXCEPTIONS_CATCHING, ResourceManager.class,
	...
	/**
	 * Formats an array of strings according to the conventions of the locale. For
	 * example, in English locales, the result is a comma-separated list with "and"
	 * preceding the last item (no commas if there are only two items). The entry in
	 * a singleton array is returned as is.
	 *
	...
	private String formatPair(AbstractResourceManager mgr, Collection items) {
		...
		result.append(
				mgr.getString(KEY_LIST_SEPARATOR_ONLY, mgr.getString(KEY_LIST_SEPARATOR, DEFAULT_LIST_SEPARATOR)));
	...
	private String formatList(AbstractResourceManager mgr, Collection items) {
		...
		final String sep = mgr.getString(KEY_LIST_SEPARATOR, DEFAULT_LIST_SEPARATOR);
	...
	/**
	 * Returns default name for the messages bundle. Subclasses should override if
	 * the messages bundle name differs from the default
	 * 
