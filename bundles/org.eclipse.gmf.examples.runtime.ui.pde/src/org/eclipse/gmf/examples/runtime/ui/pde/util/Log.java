...
public class Log {
	...
	/**
	 * Responds to a log request for the specified plug-in based on the specified
	 * status object. Statuses with severity of error or warning result in the
	 * generation of a platform log for the specified plug-in; all log requests are
	 * forward to the default log listener.
	 *
	 * @param plugin The plug-in for which to generate a log.
	...
	public static void log(Plugin plugin, IStatus status) {
		...
		default:
			/*
			 * The Eclipse logging facility does not have the concept of logging levels.
			 * Anything logged to the plugins logger will automatically be logged to the
			 * .log file. The Common Logging implementation provides the common log file
			 * (CommonBaseEvents.log) with the associated extension point and preference
			 * page but is not related to the .log file. Since we do not want to Eclipse log
			 * poluted with "plug-in started" messages, we do not log these.
			 *
			 * getDefault().log(status);
			 */
			break;
	...
	public static void log(Plugin plugin, int severity, int code, String message, Throwable throwable) {
		...
		try {
			log(plugin, new Status(severity, plugin.getBundle().getSymbolicName(), code, msg, throwable));
		} catch (IllegalArgumentException iae) {
			Trace.catching(GmfExamplesPlugin.getDefault(), GmfExamplesDebugOptions.EXCEPTIONS_CATCHING,
					Log.getDefault().getClass(), "log", iae); //$NON-NLS-1$
			Log.error(GmfExamplesPlugin.getDefault(), GmfExamplesStatusCodes.SERVICE_FAILURE, "log", iae); //$NON-NLS-1$
		}
	}

	/**
	 * Generates an error log for the specified plug-in, with the specified status
	 * code and message.
	 *
	 * @param plugin  The plug-in for which to generate a log.
	 * @param code    The status code for the log.
	...
	/**
	 * Generates a warning log for the specified plug-in, with the specified status
	 * code and message.
	 *
	 * @param plugin  The plug-in for which to generate a log.
	 * @param code    The status code for the log.
	...
	/**
	 * Generates an information log for the specified plug-in, with the specified
	 * status code and message.
	 *
	 * @param plugin  The plug-in for which to generate a log.
	 * @param code    The status code for the log.
