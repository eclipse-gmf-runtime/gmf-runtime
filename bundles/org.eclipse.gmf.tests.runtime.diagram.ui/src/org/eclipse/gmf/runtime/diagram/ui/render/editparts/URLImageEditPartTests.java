...
public class URLImageEditPartTests extends AbstractImageEditPartTests {
	...
	private URL getURL(String fileName) {
		 ...
		 * specific file
		 */
		IPath path = new Path(TRANSLATE_PATH_ARGUMENT).append(getImagePathName() + fileName);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.gmf.runtime.diagram.ui.render.editparts.
	 * AbstractImageEditPartTests#getFixture()
	 */
	@Override
	public List getFixtures() {
		...
		fixtures.add(new URLImageEditPartFixture1(getNode(), getURL(TEST2)));
		fixtures.add(new URLImageEditPartFixture1(getNode(), getURL(TEST3)));
		fixtures.add(new URLImageEditPartFixture1(getNode(), getURL(TEST4)));
		fixtures.add(new URLImageEditPartFixture1(getNode(), getURL(TEST5)));
		fixtures.add(new URLImageEditPartFixture1(getNode(), getURL(TEST6)));
	...
	protected List getNonImageFixtures() {
		...
		nonImageFixtures.add(new URLImageEditPartFixture1(getNode(), getURL(TEST7)));
		nonImageFixtures.add(new URLImageEditPartFixture1(getNode(), getURL(TEST8)));
		nonImageFixtures.add(new URLImageEditPartFixture1(getNode(), getURL(TEST9)));
		nonImageFixtures.add(new URLImageEditPartFixture1(getNode(), getURL(TEST10)));
		// nonImageFixtures.add(new URLImageEditPartFixture1(
		// getNode(), getURL(TEST11)));
		nonImageFixtures.add(new URLImageEditPartFixture1(getNode(), null)); // test null URL
	...
	public void test_regenerateImageFromSource_InvalidSource() {
		...
		while (li.hasNext()) {
			...
			if (obj instanceof AbstractImageEditPart) {
				...
				assertNull((renderedImage == null ? (Object) renderedImage : (Object) renderedImage.getSWTImage()),
						"Non image file was rendered to an image? " + (Integer.valueOf(testno++)).toString());
			}
		}
	}
}
