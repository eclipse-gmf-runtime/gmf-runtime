 ...
 * @author choang
 *
 *         AbstractConnectTests that contain the test logic that is common
 *         across diagram Should be extended by diagram specic test to test
...
public abstract class AbstractConnectionTests extends AbstractTestBase {
	...
	public void testDeleteConnection() throws Exception {
		...
		getTestFixture().getEditingDomain().runExclusive(new Runnable() {
			@Override
			public void run() {

				Diagram dgrm2 = getTestFixture().getConnectorView().getDiagram();
		...
		DeleteCommand delete = new DeleteCommand(getTestFixture().getEditingDomain(),
	...
	public void testAddBendpoints() throws Exception {
		...
		SetPropertyCommand c = new SetPropertyCommand(getTestFixture().getEditingDomain(),
				new EObjectAdapter(connectorView), Properties.ID_SMOOTHNESS, "", //$NON-NLS-1$
				s1);
		...
		newpts.addPoint(new Point(ptStart));
		newpts.addPoint(new Point(ptMiddle));
		newpts.addPoint(new Point(getDiagramEditPart().getFigure().getBounds().getBottomRight()));
		newpts.addPoint(new Point(ptEnd));
