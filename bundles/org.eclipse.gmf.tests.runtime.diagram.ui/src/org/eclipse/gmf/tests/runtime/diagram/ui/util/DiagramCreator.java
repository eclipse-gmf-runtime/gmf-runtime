	...
	static public List createNodes(final Diagram diagram, final PreferencesHint preferencesHint,
		...
		AbstractEMFOperation operation = new AbstractEMFOperation(editingDomain, StringStatics.BLANK) {
			...
			protected IStatus doExecute(IProgressMonitor monitor, IAdaptable info) throws ExecutionException {
				...
				Node note1 = ViewService.createNode(diagram, ViewType.NOTE, preferencesHint);
				Assert.isNotNull(note1, "Note1 creation failed"); //$NON-NLS-1$
				Assert.isTrue(diagram.getChildren().get(0) == note1, "Note1 is not inserted in diagram"); //$NON-NLS-1$

				Node note2 = ViewService.createNode(diagram, ViewType.NOTE, preferencesHint);
				Assert.isNotNull(note2, "Note2 creation failed"); //$NON-NLS-1$
				Assert.isTrue(diagram.getChildren().get(1) == note2, "Note2 is not inserted in diagram"); //$NON-NLS-1$

				Edge noteAttachment = ViewService.createEdge(note1, note2, ViewType.NOTEATTACHMENT, preferencesHint);
				Assert.isNotNull(noteAttachment, "NoteAttachment creation failed"); //$NON-NLS-1$
				Assert.isTrue(diagram.getEdges().get(0) == noteAttachment, "NoteAttachment is not inserted in diagram"); //$NON-NLS-1$
				...
				Assert.isTrue(noteAttachment.getSource() == note1, "Note1 is not source for noteattachment"); //$NON-NLS-1$
				Assert.isTrue(noteAttachment.getTarget() == note2, "Note2 is not target for noteattachment"); //$NON-NLS-1$

				list.add(note1);
				list.add(note2);

				return new Status(IStatus.OK, LogicDiagramPlugin.getPluginId(), DiagramStatusCodes.OK,
