...
public class ElementTypeIconProviderTest {
	...
	public void test_getIcon() {
		...
		Bundle bundle = Platform.getBundle("org.eclipse.gmf.tests.runtime.emf.type.ui"); //$NON-NLS-1$
		URL iconURL = bundle.getEntry("icons/icon.gif"); //$NON-NLS-1$
		ImageDescriptor iconImageDescriptor = ImageDescriptor.createFromURL(iconURL);
		Image icon = iconImageDescriptor.createImage();
		byte[] iconData = icon.getImageData().data;

		setFixture(new ElementTypeIconProvider());

		IElementType iconType = ElementTypeRegistry.getInstance()
				.getType("org.eclipse.gmf.tests.runtime.emf.type.ui.iconType"); //$NON-NLS-1$
	...
	/**
	 * Tests that a platform URI can be specified as the location of an element type
	 * icon.
	 */
	@Test
	public void test_getIconFromPlatformURI_144906() {

		Bundle bundle = Platform.getBundle("org.eclipse.gmf.runtime.diagram.ui.geoshapes"); //$NON-NLS-1$
		...
		IElementType iconType = ElementTypeRegistry.getInstance()
				.getType("org.eclipse.gmf.tests.runtime.emf.type.ui.platformIconType"); //$NON-NLS-1$
