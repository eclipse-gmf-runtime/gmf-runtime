...
/**
 * An AbstractFactory implementation of the <code>IViewProvider</code> interface.  
...
public class AbstractViewProvider extends AbstractProvider
	...
	/**
	 * Create an <code>IDiagramView</code> instance using the supplied parameters.  The instance is created
	 * via reflection and the supplied paramter is the view's <i>creation</i> constructor paramter.
	 */
	...
	/**
	 * Create an <code>IConnectorView</code> instance using the supplied parameters.  The instance is created
	 * via reflection and the supplied parameters are the view's <i>creation</i> constructor parameters.
	 */
	public final View createConnectorView(
	...
	/**
	 * Create an <code>IShapeView</code> instance using the supplied parameters.  The instance is created
	 * via reflection and the supplied parameters are the view's <i>creation</i> constructor parameters.
	 */
...
/*	*//**
	 * Create an instance of the view mapped to the supplied model element.  The instance is created
	 * via reflection and the supplied paramter is the view's <i>incarnate</i> constructor parameter.
	...
	public IView incarnateView(View viewElement) {
		...
		IContainerView containerView =
			(IContainerView) IncarnationUtil.incarnateView(
				ViewUtil.getContainerView(viewElement));

		// get the semantic hint
		String semanticHint = viewElement.getType(); 

		// get the semantic element
		IAdaptable semanticAdapter = viewElement.getElement() != null
			? new EObjectAdapter(viewElement.getElement()) 
			: null;
		
		if (viewElement instanceof Diagram){
			Diagram diagramView = (Diagram) viewElement;
				return createIncarnatedView(
					getIncarnateDiagramViewClass(semanticAdapter, diagramView.getType()),
					new Object[] {viewElement});
		}
		else if (viewElement instanceof Edge){
			return createIncarnatedView(
				getIncarnateConnectorViewClass(
					semanticAdapter,
					containerView,
					semanticHint),
				new Object[] {viewElement});
